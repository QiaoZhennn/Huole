...
contract HuoLe is Ownable{
  ...
  function newPost(string _msg, string _nickname, string _contact, uint _tip) public payable {
    ...
    posts_[postCount_] = Post(postCount_, now, _tip, _msg, user);
  ...
  function getPost(uint _id) public view returns(string, uint, uint, uint) {
    return (posts_[_id].text, posts_[_id].user.id, posts_[_id].postTime, posts_[_id].tip);
  }
